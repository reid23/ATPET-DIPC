...
% 
% [OUTPUTS] = dipc_mpc(INPUTS) solves an optimization problem where:
% Inputs:
% - x0 - matrix of size [140x1]
% - xinit - matrix of size [6x1]
% - all_parameters - matrix of size [140x1]
% Outputs:
...
function [x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13, x14, x15, x16, x17, x18, x19, x20] = dipc_mpc(x0, xinit, all_parameters)
    
    [output, ~, ~] = dipc_mpcBuildable.forcesCall(x0, xinit, all_parameters);
    x01 = output.x01;
    x02 = output.x02;
    x03 = output.x03;
    x04 = output.x04;
