...
def save_eoms(eoms, filename="eoms.py"):
    # save equations into a file so we can later do `from eoms import get_eoms; get_eoms()` to get the
    # equations of motion (much faster than doing the algebra all over again)
    init_vars = ""
    for i in eoms['params']:
        init_vars += f"{str(i)} = Symbol('{str(i)}');"
    init_vars += f"{str(eoms['u'])} = Symbol('{str(eoms['u'])}');"
    init_vars += f"{str(eoms['y'])} = MatrixSymbol('{str(eoms['y'])}', *{eoms['y'].shape});"
    eoms_dict = f"dict(params={str(eoms['params'])}, y={str(eoms['y'])}, u={str(eoms['u'])}, ode={str(eoms['ode'])})"
    with open(filename, 'w') as f:
        print("# This file was auto-generated by `dynamics.py`", file=f)
        print("from sympy import *", file=f)
        print(f"def get_eoms(): {init_vars} return {eoms_dict}", file=f)
...
if __name__ == '__main__':
    ...
    u = lambda t: np.cos(int(t/0.02)*0.02*5)*0.2
    tgrid = np.linspace(0, 5, 100, dtype=float).flatten().tolist()
    ca_intfunc = ca.integrator('intfunc', 'idas', ca_eoms, 0, tgrid)
    Zip = perf_counter()
    fun = lambda t, x: f(u(t), np.array(x).reshape((6, 1))).T
    res = sp.integrate.solve_ivp(fun, [0, 5], x0, rtol=1e-8, atol=1e-8, t_eval=tgrid)
    Zap = perf_counter()
    res2 = ca_intfunc(x0=x0, p=p, u=[u(t) for t in tgrid])['xf']
    Zop = perf_counter()
    print(f'scipy integration took {Zap-Zip:.5f}s')
    print(f'casadi integration took {Zop-Zap:.5f}s')
    # plotting stuff
    if True:
    ...
    # linearize the system and check controllability
    a, b = A(0, np.zeros((6, 1))), B(0, np.zeros((6, 1)))
    ctrb_rank = np.linalg.matrix_rank(np.hstack([b, a@b, a@a@b, a@a@a@b, a@a@a@a@b, a@a@a@a@a@b]))
    print(f"ctrb matrix rank: {ctrb_rank}")
# %%
