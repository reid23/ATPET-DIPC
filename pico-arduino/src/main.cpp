...
#define MAX_VEL 5000
#define MAX_ACC 20000000 // all accelerations are 1000x bigger
...
#define LOOP_PERIOD 1000 // microseconds, minimum
...
void update_encoder_data() {
  ...
  digitalWrite(CS_PIN, LOW);
  uint16_t topread = SPI1.transfer16(read_angle);
  uint16_t endread = SPI1.transfer16(read_angle);
  digitalWrite(CS_PIN, HIGH);
  SPI1.endTransaction();
  delayMicroseconds(1);
  if ((topread & (uint16_t)0b0100000000000000) > 0) { clear_error_flag(); topread = top; }
  if ((endread & (uint16_t)0b0100000000000000) > 0) { clear_error_flag(); endread = end; }
  oldtop = top;
  oldend = end;
