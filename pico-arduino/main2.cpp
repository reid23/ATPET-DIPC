...
static float endvelbuf[BUF_LEN] = {0.0};
...
void update_encoder_data() {
  ...
  endvelbuf[bufptr] = endmed.getMedian();
  bufptr++;
  ...
  for (int i = 0; i<BUF_LEN; i++) {
    ...
    endpos += endvelbuf[(bufptr+i)%BUF_LEN]*pos_weights[i];
    topvel += topbuf[(bufptr+i)%BUF_LEN]*vel_weights[i];
    endvel += endvelbuf[(bufptr+i)%BUF_LEN]*vel_weights[i];
  }
