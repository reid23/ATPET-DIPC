...
use hal::fugit::{RateExtU32, ExtU32}; // for .kHz() and similar
...
static mut DIR_PIN: Option<Pin<hal::gpio::bank0::Gpio10, hal::gpio::FunctionSioOutput, PushPullOutput>> = None;
...
static mut RESET_PIN: Option<Pin<hal::gpio::bank0::Gpio11, hal::gpio::FunctionSioInput, PullUpInput>> = None;
...
const SWING_UP_PROG: [f32; 24] = [2.575035906172058, 2.575035906172058, -3.8158905593996133, -3.8158905593996133, -2.0671993855312296, -2.0671993855312296, 4.139213882689204, 4.139213882689204, 2.2243153434826377, 2.2243153434826377, 1.4948059154622, 1.4948059154622, -4.421279471702257, -4.421279471702257, -2.082844104057677, -2.082844104057677, -1.819244414580912, -1.819244414580912, 4.242038184131376, 4.242038184131376, 2.3720656176437367, 2.3720656176437367, -3.938846230847921, -3.938846230847921];
const SWING_DOWN_PROG: [f32; 17] = [-3.385853038570042, -2.291104168320536, 4.1975310847926535, 3.8602780696650716, 2.631301215914938, -4.386163191848309, -3.5911054562968814, -2.4652948343294234, -1.8780577501933995, 1.0977526545032275, 3.883398002439094, 4.100956276604702, 2.813741073637396, 2.0584780031239647, 0.585821960770236, -2.955463562063751, -4.426063147664362];
...
fn main() -> ! {
    ...
    // Set gpio10 (step pin) to pio
    // let _step: hal::gpio::Pin<_, hal::gpio::FunctionPio1, PushPullOutput> = pins.gpio5.into_mode();
    let _step = pins.gpio5.into_push_pull_output();
    ...
    // Set pio pindir for gpio10
    step_sm.set_pindirs([(step_pin_id, hal::pio::PinDir::Output)]);
    
    let mut dir_pin = pins.gpio10.into_push_pull_output();
