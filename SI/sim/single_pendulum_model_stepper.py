...
class dipc_model:
    def __init__(self, constants={ # [0.15, 40, 9.8, 0.125, 0.125, 1.74242, 0.04926476]
        ...
        # self.cart.apply_force(-(self.ma*self.F + 0.5*self.l*self.mb*-9.8*sp.sin(2*self.y[1]) - self.mb*self.l*(self.y[3]**2)*sp.sin(self.y[1]))*self.cart.x)
...
if __name__ == '__main__':
    ...
    model.integrate_with_scipy(
        ...
        targets = np.array([[  0, np.pi, 0, 0],
                            [0.5, np.pi, 0, 0]]),
                            controller = lambda t: 0, lag_seconds=0.0165)
    ...
    #%%
    model.soln_t = np.arange(0, 15, 1/60)
