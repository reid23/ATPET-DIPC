...
data2[:, 1] /= 1000.0
data2[:, 2] /= 1000.0
data2 = np.load('../../me-c231a/data2.npy')
data2 = data2[:, 1:]
data2[:, 0]*=1e6
...
x0 = [0.139628, 0.190044, 0.143263, 0.00267596, 0.0013376, 133049, 0, 0]
lbx = [0.05, 0.01,  0.02, 0.0001, 0.0001,  0.01, 0, 0]
ubx = [0.3,  0.3,   0.2,   0.05,   0.05,  0.2, 0, 0]
...
#%%
plot(np.array(soln['x']).flatten(), 00)
print(soln)
# %%
